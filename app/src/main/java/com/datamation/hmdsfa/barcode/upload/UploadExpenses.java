...
public class UploadExpenses extends AsyncTask<ArrayList<DayExpHed>, Integer, ArrayList<DayExpHed>> {
    ...
    List<String> resultListExpenses;
    ...
    protected void onPostExecute(ArrayList<DayExpHed> RCSList) {
        ...
        dialog.dismiss();
        taskListener.onTaskCompleted(taskType,resultListExpenses);
    }
    private void addRefNoResults(String ref, int count) {
        resultListExpenses.add(ref);
        if(count == resultListExpenses.size()) {
            mUploadResult(resultListExpenses);
        }
    }
    ...
    public void mUploadResult(List<String> messages) {
        ...
        alertDialogBuilder.setTitle("Upload expense summary");
