package com.datamation.hmdsfa.vansale;
...
public class UploadDeletedInvoices extends AsyncTask<ArrayList<InvHed>, Integer, ArrayList<InvHed>> {
    ...
    public UploadDeletedInvoices(Context context, UploadTaskListener taskListener) {
    ...
    protected ArrayList<InvHed> doInBackground(ArrayList<InvHed>... params) {
        ...
        for (InvHed c : RCSList) {
            ...
            try {
                ...
                boolean bStatus = NetworkFunctions.mHttpManager(networkFunctions.syncDeletedInvoice(),List.toString());
    ...
    protected void onPostExecute(ArrayList<InvHed> RCSList) {
        ...
        int i = 1;
        for (InvHed c : RCSList) {
            new InvHedController(context).updateIsSyncedLogTbl(c);
