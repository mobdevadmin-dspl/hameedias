...
public class ReceiptController {
	...
	Context context;
	private String TAG = "ReceiptController";
// rashmi - 2019-12-18 move from database_helper , because of reduce coding in database helper*******************************************************************************
	...
	public static final String CREATE_FPRECHED_TABLE = "CREATE  TABLE IF NOT EXISTS " + TABLE_FPRECHED + " (" + FPRECHED_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
			...
			ValueHolder.REFNO + " TEXT, " + FPRECHED_REFNO1 + " TEXT, " + FPRECHED_MANUREF + " TEXT, " + FPRECHED_SALEREFNO + " TEXT, " +

			ValueHolder.REPCODE + " TEXT, " + FPRECHED_TXNTYPE + " TEXT, " + FPRECHED_CHQNO + " TEXT, " + FPRECHED_CHQDATE + " TEXT, " + ValueHolder.TXNDATE + " TEXT, " + FPRECHED_CURCODE + " TEXT, " +

			FPRECHED_CURRATE1 + " TEXT, " + ValueHolder.DEBCODE + " TEXT, " + FPRECHED_TOTALAMT + " TEXT, " + FPRECHED_BANKCODE + " TEXT, " + FPRECHED_BRANCHCODE + " TEXT, " +
	...
	public static final String CREATE_FPRECHEDS_TABLE = "CREATE  TABLE IF NOT EXISTS " + TABLE_FPRECHEDS + " (" + FPRECHED_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
			...
			FPRECHED_RECORDID + " TEXT, " + FPRECHED_TIMESTAMP + " TEXT, " + FPRECHED_ISDELETE + " TEXT, " + FPRECHED_COST_CODE + " TEXT, " + FPRECHED_COMMON_REFNO + " TEXT, "
	...
	public int createOrUpdateRecHed(ArrayList<ReceiptHed> list) {
		...
		try {
			...
			for (ReceiptHed recHed : list) {
				...
				//values.put(ValueHolder.REFNO2, recHed.getFPRECHED_REFNO2());
				...
				if (cn > 0) {
					count = dB.update(TABLE_FPRECHED, values, ValueHolder.REFNO + " =?",
	...
	public int updateIsSynced(String refno,String res) {
		...
		try {
			...
			count = dB.update(TABLE_FPRECHEDS, values, ValueHolder.REFNO + " =?", new String[] { refno });
//            }else{
...
//            }
	...
	public int createOrUpdateRecHedS(ArrayList<ReceiptHed> list) {
		...
		try {
			...
			for (ReceiptHed recHed : list) {
				...
				values.put(FPRECHED_COMMON_REFNO, recHed.getFPRECHED_COMMON_RENNO());

				//	values.put(FPRECHED_ADDUSER_NEW, recHed.getFPRECHED_ADDUSER_NEW());

				int cn = cursor.getCount();
				if (cn > 0) {
					count = dB.update(TABLE_FPRECHEDS, values, ValueHolder.REFNO + " =?",
							new String[] { String.valueOf(recHed.getFPRECHED_REFNO()) });
				} else {
		...
		try {
			if (refno.equals("")) {
				...
				selectQuery = "select * from " + TABLE_FPRECHEDS + " Where "
						...
						+ "='0' Order by "+FPRECHED_ISSYNCED + "," +ValueHolder.REFNO + " DESC";

			} else {

				selectQuery = "select * from " + TABLE_FPRECHEDS + " Where "
						+ FPRECHED_ISDELETE + "='0' and "+ FPRECHED_ISACTIVE + "='0' and " + ValueHolder.REFNO + "='" + refno
						+ "' Order by "+FPRECHED_ISSYNCED + "," +ValueHolder.REFNO + " DESC";
			}
	...
	public ReceiptHed getReceiptByRefno(String refno) {
		...
		try {
			...
			selectQuery = "select * from " + TABLE_FPRECHEDS + " Where " + ValueHolder.REFNO
					+ "='" + refno + "'";
	...
	public int CancelReceipt(String Refno) {
		...
		int result = 0;
		try {
			result = dB.delete(TABLE_FPRECHED, ValueHolder.REFNO + "=?",
					new String[] { Refno });
	...
	public void UpdateRecHed(ReceiptHed recHed, String Refno) {
		...
		try {
			...
			dB.update(TABLE_FPRECHEDS, values, ValueHolder.REFNO + " =?",
					new String[] { String.valueOf(Refno) });
	...
	public String getFPRECHED_REFNO2ForRecDet(String code, String status) {
		...
		String selectQuery = "SELECT * FROM " + TABLE_FPRECHED + " WHERE " + ValueHolder.DEBCODE
...
//						new String[] { refno });
//				result2=result2+1;
//				//String query = "Delete FROM " + ValueHolder.TABLE_FPRECHED + " where refno='" + refno + "'";
//				//dB.execSQL(query);
//			}
...
//		try {
//
//				String query = "Delete from " + ValueHolder.TABLE_FPRECHED+ " where totalamt is null ";
//				dB.execSQL(query);						
//
//		}
		catch (Exception e) {
...
//				mapper.setSALEREP_AREACODE(new SalRepDS(context).getAreaCode());
	...
	public ReceiptHed getActiveRecHed() {
		...
		String selectQuery = "select * from " + TABLE_FPRECHEDS + " Where " + FPRECHED_ISACTIVE + "='1' and " + FPRECHED_ISSYNCED + "='0'";
		Cursor cursor = dB.rawQuery(selectQuery, null);
		ReceiptHed recHed = new ReceiptHed();
	...
	public boolean isAnyActiveRecHed() {
		...
		Cursor cursor = null;
		try {
			String selectQuery = "SELECT * FROM " + TABLE_FPRECHED + " WHERE " + FPRECHED_ISACTIVE + "='1'";
			cursor = dB.rawQuery(selectQuery, null);
	...
	public ArrayList<ReceiptHed> getHedRefNoByCommonRef(String comRefNo) {
		if (dB == null) {
			open();
		} else if (!dB.isOpen()) {
			open();
	...
	//------------------kaveesha ------------------19/11/2020---------------------------
	public ReceiptHed getReceiptByCommnRefNo(String CommnRefNo) {
		if (dB == null) {
			open();
		} else if (!dB.isOpen()) {
			open();
		...
		try {
			...
			selectQuery = "select * from " + TABLE_FPRECHEDS + " Where " + FPRECHED_COMMON_REFNO
					+ "='" + CommnRefNo + "'";
			...
			while (cursor.moveToNext()) {
				...
				recHed.setFPRECHED_TXNDATE(cursor.getString(cursor.getColumnIndex(ValueHolder.TXNDATE)));
				recHed.setFPRECHED_TXNTYPE(cursor.getString(cursor.getColumnIndex(FPRECHED_TXNTYPE)));
				recHed.setFPRECHED_COMMON_RENNO(cursor.getString(cursor.getColumnIndex(FPRECHED_COMMON_REFNO)));
				//	recHed.setFPRECHED_ADDUSER_NEW(cursor.getString(cursor.getColumnIndex(FPRECHED_ADDUSER_NEW)));
