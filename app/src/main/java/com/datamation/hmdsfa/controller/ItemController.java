...
import com.datamation.hmdsfa.model.Group;
...
//            items.setFITEM_VEN_P_CODE(cursor.getString(cursor.getColumnIndex(dbHelper.FITEM_VEN_P_CODE)));
    ...
    public Item getItem(String itemCode) {
        ...
        Item items=new Item();
        //  String selectQuery = "SELECT *  FROM fItem WHERE ItemCode LIKE '%"+itemCode+"%'";
        String selectQuery =  "SELECT itm.*, loc.QOH FROM fitem itm, fitemLoc loc WHERE  itm.ItemCode = '" + itemCode + "' AND loc.ItemCode=itm.ItemCode ORDER BY CAST(loc.QOH AS FLOAT) DESC";
...
//-----------kaveesha------------28/08/2020----------To get Product Group wise stock------------------------
    public ArrayList<StockInfo> getGwiseStocks(String newText, String LocCode) {
        if (dB == null) {
            open();
        } else if (!dB.isOpen()) {
            open();
